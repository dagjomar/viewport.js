...
(function(window) {
    ...
    runner.prototype.runPage = function() {
        ...
        test.frame = $('<iframe class="runner-test-page-frame" src="' + test.page + '" width="100%" height="600"></iframe>')[0];
    ...
    runner.prototype.pageProgress = function(frame, testResults, total, testName, isDone) {
        console.log(testResults);
        var that = this;
        $.grep(this.tests, function(test, index) {
            if (test.frame === frame) {
                $(test.header).removeClass("passed").addClass( (testResults && testResults.failed) > 0 ? "failed" : "passed")
                            .html(test.title + " | " + testName + ((testResults && testResults.total) == 0 ? "" : "  " + (testResults && testResults.failed) + " failed, " + (testResults && testResults.total) + " total"));
                if (isDone) {
                    test.complete = true;
                    that.failures += (testResults && testResults.failed);
                    that.total += total;
